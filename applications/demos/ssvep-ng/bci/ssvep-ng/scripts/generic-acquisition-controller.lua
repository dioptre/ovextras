...
function initialize(box)
	...
	-- get the targets' materials
	
	s_targetMaterials = box:get_setting(8)
	
	for s_material in s_targetMaterials:gmatch("[^;]+") do
		box:log("Info", string.format("Material : %s", s_material))
		table.insert(target_materials, s_material);
	end
	...
	for target_index, position in ipairs(target_positions) do
		...
		cfg_file:write("SSVEP_Target_Material_", target_index - 1, " = ", target_materials[target_index], "\n")
	end
