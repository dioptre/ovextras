...
{
    CConfigurationGTecGUSBampLinux* l_pConfig=static_cast<CConfigurationGTecGUSBampLinux*>(pUserData);
    l_pConfig->OnComboboxDeviceChanged();
}
...
void CConfigurationGTecGUSBampLinux::UpdateFilters(void)
{
...
boolean CConfigurationGTecGUSBampLinux::preConfigure(void)
{
    ...
    for(gboolean l_bEnd = gtk_tree_model_get_iter_first(l_pTreeModelName, &l_oIterName); l_bEnd; l_bEnd = gtk_tree_model_iter_next(l_pTreeModelName, &l_oIterName))
    {
        ...
        if(*m_pDeviceName == l_oFilterDesc)		// todo: check that this works with the API
        {
...
{    
    m_rDriverContext.getLogManager() << LogLevel_Warning << "Changing sampling frequency invalidates filters\n";
...
{
    m_rDriverContext.getLogManager() << LogLevel_Warning << "Changing device invalidates filters\n";
