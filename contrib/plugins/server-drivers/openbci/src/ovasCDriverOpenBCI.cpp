...
void CDriverOpenBCI::updateDaisy(bool bQuietLogging)
{
	// change channel and sampling rate according to daisy module
	CConfigurationOpenBCI::SDaisyInformation l_oDaisyInformation=CConfigurationOpenBCI::getDaisyInformation(m_bDaisyModule?CConfigurationOpenBCI::DaisyStatus_Active:CConfigurationOpenBCI::DaisyStatus_Inactive);
	...
	IDriverCallback& rCallback)
{
	...
	// init state
	m_ui32ReadState=CDriverOpenBCI::ParserAutomaton_Default;
...
#if 0
	uint32 l_f32UnitsToMicroVolts = (float32) (ADS1299_VREF/(pow(2.,23)-1)/ADS1299_GAIN*1000000.); // $$$$ The notation here is ambiguous
...
int32 CDriverOpenBCI::interpret24bitAsInt32(const std::vector < uint8 >& byteBuffer)
{
...
int32 CDriverOpenBCI::interpret16bitAsInt32(const std::vector < uint8 >& byteBuffer)
{
	// create a big endian so that we could adapt to host architecture later on
...
OpenViBE::int16 CDriverOpenBCI::parseByte(uint8 ui8Actbyte)
{
	...
	switch(m_ui32ReadState)
	{
		...
		case CDriverOpenBCI::ParserAutomaton_Default:
			...
			else
			{
				...
				{
					m_ui32ReadState = CDriverOpenBCI::ParserAutomaton_StartByteReceived;
				}
		...
		case CDriverOpenBCI::ParserAutomaton_StartByteReceived:
			m_i16SampleNumber = ui8Actbyte;
			m_ui32ReadState = CDriverOpenBCI::ParserAutomaton_SampleNumberReceived;
			break;
		 ...
		 */
		case CDriverOpenBCI::ParserAutomaton_SampleNumberReceived:
			...
			if(m_ui32ExtractPosition == EEGValueCountPerSample)
			{
				...
				m_ui32ReadState = CDriverOpenBCI::ParserAutomaton_EEGSamplesReceived;
		 ...
		 */
		case CDriverOpenBCI::ParserAutomaton_EEGSamplesReceived:
			...
			{
				// next step: footer
				m_ui32ReadState = CDriverOpenBCI::ParserAutomaton_AccelerometerSamplesReceived;
		...
		case CDriverOpenBCI::ParserAutomaton_AccelerometerSamplesReceived:
...
boolean CDriverOpenBCI::sendCommand(::FD_TYPE i32FileDescriptor, const char* sCommand, bool bWaitForResponse, bool bLogResponse, uint32 ui32Timeout, std::string& sReply)
{
	...
	{
		m_rDriverContext.getLogManager() << LogLevel_Trace << this->m_sDriverName << ": Sending sequence to OpenBCI board [" << std::string(1, sCommand[i]).c_str() << "]\n";
...
boolean CDriverOpenBCI::resetBoard(::FD_TYPE i32FileDescriptor, bool bRegularInitialization)
{
	...
	if(bRegularInitialization)
	{
		...
		while(std::getline(l_oCommandReplyInputStringStream, l_sLine, '\n'))
		{
			if(l_sLine != "$$$")
			{
				::sscanf(l_sLine.c_str(), "OpenBCI V%u %u channel", &m_oDeviceInformation.ui32DeviceVersion, &m_oDeviceInformation.ui32DeviceChannelCount);
		...
		{
			m_rDriverContext.getLogManager() << LogLevel_Error << this->m_sDriverName << ": The attached device does not look like an OpenBCI device\n";
			return false;
		}
...
bool CDriverOpenBCI::handleCurrentSample(int32 i32PacketNumber)
{
...
boolean CDriverOpenBCI::openDevice(::FD_TYPE* pFileDescriptor, uint32 ui32TTYNumber)
{
	CString l_sTTYName;
	...
	if(ui32TTYNumber == UNDEFINED_DEVICE_IDENTIFIER)
	{
		...
		do
		{
			l_sTTYName = CConfigurationOpenBCI::getTTYFileName(i++);
			if(CConfigurationOpenBCI::isTTYFile(l_sTTYName))
			{
				m_rDriverContext.getLogManager() << LogLevel_Info << this->m_sDriverName << ": Automatically picked port [" << i << ":" << l_sTTYName << "]\n";
				l_bSuccess=true;
			}
		}
		while(!l_bSuccess && i<CConfigurationOpenBCI::getMaximumTTYCount());
	...
	{
 		l_sTTYName = CConfigurationOpenBCI::getTTYFileName(ui32TTYNumber);
	}
...
void CDriverOpenBCI::closeDevice(::FD_TYPE i32FileDescriptor)
{
#if defined TARGET_OS_Windows
...
uint32 CDriverOpenBCI::writeToDevice(::FD_TYPE i32FileDescriptor, const void* pBuffer, const uint32 ui32BufferSize)
{
...
uint32 CDriverOpenBCI::readFromDevice(::FD_TYPE i32FileDescriptor, void* pBuffer, const uint32 ui32BufferSize, const uint64 ui64TimeOut)
{
#if defined TARGET_OS_Windows
