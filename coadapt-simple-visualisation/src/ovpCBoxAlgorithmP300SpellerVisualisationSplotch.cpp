...
boolean CBoxAlgorithmP300SpellerVisualisationSplotch::initialize(void)
{
	...
	m_pWronglySelectedFontDescription=NULL;
	...
	m_oWronglySelectedBackgroundColor   =_AutoCast_(l_rStaticBoxContext, this->getConfigurationManager(), 15);
	m_oWronglySelectedForegroundColor   =_AutoCast_(l_rStaticBoxContext, this->getConfigurationManager(), 16);
	m_ui64WronglySelectedFontSize       =FSettingValueAutoCast(*this->getBoxAlgorithmContext(), 17);
	OpenViBE::uint64 l_ui64ResultFontSize       =FSettingValueAutoCast(*this->getBoxAlgorithmContext(), 18);

	CString l_sLettersConfigurationFilename(FSettingValueAutoCast(*this->getBoxAlgorithmContext(), 19));
	...
	m_pWronglySelectedFontDescription=pango_font_description_copy(pango_context_get_font_description(gtk_widget_get_pango_context(m_pMainWindow)));
	...
	l_ui64MaxSize=std::max(l_ui64MaxSize, m_ui64WronglySelectedFontSize);
	...
	pango_font_description_set_size(m_pWronglySelectedFontDescription, m_ui64WronglySelectedFontSize * PANGO_SCALE);
	...
	this->_cache_for_each_(&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_background_cb_, &m_oNoFlashBackgroundColor);
	this->_cache_for_each_(&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_foreground_cb_, &m_oNoFlashForegroundColor);
	this->_cache_for_each_(&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_font_cb_, l_pMaxFontDescription);
	...
	initializeFlashGroupDictionary(l_sLettersConfigurationFilename);

	return true;
}

void CBoxAlgorithmP300SpellerVisualisationSplotch::initializeFlashGroupDictionary(OpenViBE::CString filename)
{
	...
	/*
	for(FlashDictionaryType::iterator itMap=flashGroups.begin();itMap!=flashGroups.end();itMap++)
	{
		for(FlashGroupType::iterator itList=(itMap->second).begin();itList!=(itMap->second).end();itList++)
		{
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::openChild(const char* sName, const char** sAttributeName, const char** sAttributeValue, XML::uint64 ui64AttributeCount)
{
	m_vNode.push(sName);
}

void CBoxAlgorithmP300SpellerVisualisationSplotch::processChildData(const char* sData)
{
	...
	if(m_vNode.top()==CString("Group"))
	{
		...
		while(!l_sData.eof())
		{
			...
			//convert the letter value to its coordinate
			uint32 l_ui32Column = (l_ui64LetterIdValue-1)%m_ui64ColumnCount;
			uint32 l_ui32Row = (l_ui64LetterIdValue - l_ui32Column)/m_ui64RowCount;
			//
			std::cout << l_ui64LetterIdValue << "=>" << l_ui32Row <<", "<<l_ui32Column<<"\n";
			//insert in the current flash group
			flashGroup.push_front(std::make_pair((unsigned long)l_ui32Row,(unsigned long)l_ui32Column));
			
		}
		//insert in the global dictionnary
		flashGroups[m_ui32LetterGroupId] = flashGroup;
	}
	//*/

}

void CBoxAlgorithmP300SpellerVisualisationSplotch::closeChild(void)
{
	m_vNode.pop();
}

boolean CBoxAlgorithmP300SpellerVisualisationSplotch::uninitialize(void)
{
	if(m_pWronglySelectedFontDescription)
	{
		pango_font_description_free(m_pWronglySelectedFontDescription);
		m_pWronglySelectedFontDescription=NULL;
	}
...
boolean CBoxAlgorithmP300SpellerVisualisationSplotch::processInput(uint32 ui32Index)
{
	...
	if(!m_bTableInitialized)
	{
		this->_cache_for_each_(&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_background_cb_, &m_oNoFlashBackgroundColor);
		this->_cache_for_each_(&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_foreground_cb_, &m_oNoFlashForegroundColor);
		this->_cache_for_each_(&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_font_cb_, m_pNoFlashFontDescription);
...
boolean CBoxAlgorithmP300SpellerVisualisationSplotch::process(void)
{
	// IBox& l_rStaticBoxContext=this->getStaticBoxContext();
			...
			for(j=0; j<l_pStimulationSet->getStimulationCount(); j++)
			{
				...
				if(l_bFlash)
				{
					this->_cache_for_each_if_(
						l_iRow,
						l_iColumn,
						&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_background_cb_,
						&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_background_cb_,
					...
					this->_cache_for_each_if_(
						l_iRow,
						l_iColumn,
						&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_foreground_cb_,
						&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_foreground_cb_,
					...
					this->_cache_for_each_if_(
						l_iRow,
						l_iColumn,
						&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_font_cb_,
						&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_font_cb_,
				...
				for(j=0; j<l_pStimulationSet->getStimulationCount(); j++)
				{
					...
					if(l_bTarget && m_iTargetRow!=-1 && m_iTargetColumn!=-1)
					{
						...
						this->_cache_for_each_if_(
							...
							m_iTargetColumn,
							&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_background_cb_,
							&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_null_cb_,
						...
						this->_cache_for_each_if_(
							m_iTargetRow,
							m_iTargetColumn,
							&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_collect_child_widget_cb_,
							&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_collect_child_widget_cb_,
							&l_vWidgets,
							NULL);
						...
						if(l_vWidgets.size() == 1)
						{
							if(GTK_IS_LABEL(l_vWidgets[0]))
							{
								...
								std::string l_sTargetChar = gtk_label_get_text(GTK_LABEL(l_vWidgets[0]));
					...
					for(j=0; j<l_pStimulationSet->getStimulationCount(); j++)
					{
						...
						if(l_bSelected && m_iSelectedRow!=-1 && m_iSelectedColumn!=-1)
						{
							if(m_iSelectedRow>=0 && m_iSelectedColumn>=0)
							{
								...
								boolean l_bHalfCorrect = false;
								if(m_vTargetHistory.size()) {
									std::list < std::pair < int, int > >::const_iterator it=m_vTargetHistory.begin();
									l_bCorrect=(it->first==m_iSelectedRow && it->second==m_iSelectedColumn);
									l_bHalfCorrect=(it->first==m_iSelectedRow || it->second==m_iSelectedColumn);
									...
									else
									{
										this->_cache_for_each_if_(
											...
											&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_background_cb_,
											&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_null_cb_,
											&m_oWronglySelectedBackgroundColor,
											NULL);
										this->_cache_for_each_if_(
											m_iLastTargetRow,
											m_iLastTargetColumn,
											&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_foreground_cb_,
											&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_null_cb_,
											&m_oWronglySelectedForegroundColor,
											NULL);
										this->_cache_for_each_if_(
											m_iLastTargetRow,
											m_iLastTargetColumn,
											&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_font_cb_,
											&CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_null_cb_,
											m_pWronglySelectedFontDescription,
											NULL);
									}
								}
								else
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_build_from_table_(::GtkTable* pTable)
{
	if(pTable)
	{
		::GdkColor l_oWhite;
		...
		for(l_pList=pTable->children; l_pList; l_pList=l_pList->next)
		{
			...
			for(unsigned long i=l_pTableChild->top_attach; i<l_pTableChild->bottom_attach; i++)
			{
				...
				{
					CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle& l_rWidgetStyle=m_vCache[i][j];
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_for_each_(_cache_callback_ fpCallback, void* pUserData)
{
	std::map < unsigned long, std::map < unsigned long, CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle > >::iterator i;
	std::map < unsigned long, CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle >::iterator j;
...
bool CBoxAlgorithmP300SpellerVisualisationSplotch::isMemberOfGroup(bool rowGroup, unsigned long row_col_id, unsigned long row, unsigned long col)
{
	bool ismember = false;

	if (!rowGroup)
	{
		row_col_id = row_col_id + 6;
	}
	unsigned long groupId = row_col_id;

	FlashGroupType::iterator listIt = flashGroups[groupId].begin();
	for (; listIt!=flashGroups[groupId].end();listIt++)
	{
		if(listIt->first==row && listIt->second==col)
		{
			ismember = true;
		}
	}
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_for_each_if_(int iLine, int iColumn, _cache_callback_ fpIfCallback, _cache_callback_ fpElseCallback, void* pIfUserData, void* pElseUserData)
{
	std::map < unsigned long, std::map < unsigned long, CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle > >::iterator i;
	std::map < unsigned long, CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle >::iterator j;
	...
	for(i=m_vCache.begin(); i!=m_vCache.end(); i++)
	{
		for(j=i->second.begin(); j!=i->second.end(); j++)
		{
			...
			if(l_bLine && isMemberOfGroup(true,(unsigned long)iLine, i->first, j->first))
			{
				l_bInLine=true;
			}
			if(l_bColumn && isMemberOfGroup(false,(unsigned long)iColumn, i->first, j->first))
			{
				l_bInColumn=true;
			}
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_null_cb_(CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle& rWidgetStyle, void* pUserData)
{
}

void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_background_cb_(CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle& rWidgetStyle, void* pUserData)
{
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_foreground_cb_(CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle& rWidgetStyle, void* pUserData)
{
	::GdkColor oColor=*(::GdkColor*)pUserData;
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_change_font_cb_(CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle& rWidgetStyle, void* pUserData)
{
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_collect_widget_cb_(CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle& rWidgetStyle, void* pUserData)
{
	if(pUserData)
	{
...
void CBoxAlgorithmP300SpellerVisualisationSplotch::_cache_collect_child_widget_cb_(CBoxAlgorithmP300SpellerVisualisationSplotch::SWidgetStyle& rWidgetStyle, void* pUserData)
{
	if(pUserData)
	{
