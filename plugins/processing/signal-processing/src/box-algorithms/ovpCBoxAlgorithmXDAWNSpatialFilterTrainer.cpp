...
boolean CBoxAlgorithmXDAWNSpatialFilterTrainer::initialize(void)
{
	...
	m_oSpatialFilterEncoder.initialize(*this);
...
boolean CBoxAlgorithmXDAWNSpatialFilterTrainer::uninitialize(void)
{
	...
	m_oSpatialFilterEncoder.uninitialize();
	...
	if(l_bShouldTrain)
	{
		...
		for(i=0; i<l_rDynamicBoxContext.getInputChunkCount(1); i++)
		{
			...
			if(m_pSignalDecoder->isOutputTriggerActive(OVP_GD_Algorithm_SignalStreamDecoder_OutputTriggerId_ReceivedBuffer))
			{
				...
				/*if (l_ui64EndTime==uint64(-1))
				{
					l_ui64EndTime = l_oChunk.m_ui64EndTime;
				}
				else*/
					if(l_ui64EndTime!=uint64(-1) && l_oChunk.m_ui64StartTime!=l_ui64EndTime)
					{
		...
		this->getLogManager() << LogLevel_Trace << "Decoding evoked response potential, " << l_rDynamicBoxContext.getInputChunkCount(2) << " chunks received\n";
		...
		for(i=0; i<l_rDynamicBoxContext.getInputChunkCount(2); i++)
		{
			...
			if(m_pEvokedPotentialDecoder->isOutputTriggerActive(OVP_GD_Algorithm_SignalStreamDecoder_OutputTriggerId_ReceivedBuffer))
			{
				...
				//this->getLogManager() << LogLevel_Info << "Dimensions evoked response epoch " << l_oChunk.m_pMatrix->getDimensionSize(0) << "," << l_oChunk.m_pMatrix->getDimensionSize(1) << "\n";
 				//this->getLogManager() << LogLevel_Info << "First and second element of evoked response epoch " << *l_oChunk.m_pMatrix->getBuffer() << "," << *(l_oChunk.m_pMatrix->getBuffer()+1) << "\n";				
			}
		...
		this->getLogManager() << LogLevel_Info << "Number of Evoked potential epochs  " << uint64(l_vEvokedPotential.size()) << "\n";				
		...
		OpenViBEToolkit::Tools::Matrix::copy(
			...
			*it->m_pMatrix);
		//this->getLogManager() << LogLevel_Info << "Dimensions evoked average response epoch " << l_oAveragedERPMatrixOV.getDimensionSize(0) << "," << l_oAveragedERPMatrixOV.getDimensionSize(1) << "\n";
		...
		//this->getLogManager() << LogLevel_Info << "Converted to IT++ matrices\n";
		...
		if(itppextCoadapt::eig(A, B, l_oEigenValue, l_oEigenVector))
		{
			...
			std::map < double, itpp::vec >::const_reverse_iterator rit;
			...
			for(rit=l_vEigenVector.rbegin(), i=0; rit!=l_vEigenVector.rend() && i<m_ui64FilterDimension; rit++, i++)
			{
				for(j=0; j<l_ui32ChannelCount; j++)
				{
					::fprintf(l_pFile, "%e ", rit->second[j]);
				}
			}
			::fprintf(l_pFile, "</SettingValue>\n");
			...
			for(rit=l_vEigenVector.rbegin(), i=0; rit!=l_vEigenVector.rend() && i<m_ui64FilterDimension; rit++, i++)
			{
				this->getLogManager() << " | " << float64(rit->first);
			}
			this->getLogManager() << "\n";
		}
		else
		...
		IMatrix* l_pSpatialFilter = m_oSpatialFilterEncoder.getInputMatrix();
		l_pSpatialFilter->setDimensionCount(2);
		l_pSpatialFilter->setDimensionSize(0,uint32(m_ui64FilterDimension));
		l_pSpatialFilter->setDimensionSize(1,l_ui32ChannelCount);
		OpenViBEToolkit::Tools::Matrix::clearContent(*l_pSpatialFilter);
		m_oSpatialFilterEncoder.encodeHeader(1);
		...
		for(rit=l_vEigenVector.rbegin(), i=0; rit!=l_vEigenVector.rend() && i<m_ui64FilterDimension; rit++, i++)
			{
				for(j=0; j<l_ui32ChannelCount; j++)
				{
					*(l_pSpatialFilter->getBuffer()+k++) = rit->second[j];
				}
			}
		m_oSpatialFilterEncoder.encodeBuffer(1);
		l_rDynamicBoxContext.markOutputAsReadyToSend(1,l_ui64TrainChunkStartTime, l_ui64TrainChunkEndTime);
