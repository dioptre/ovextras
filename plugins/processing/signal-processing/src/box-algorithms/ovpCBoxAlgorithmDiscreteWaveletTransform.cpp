...
boolean CBoxAlgorithmDiscreteWaveletTransform::process(void)
{
	...
	int J = std::atoi(m_sDecompositionLevel);
	...
	uint32 l_ui32NbChannels0;
	uint32 l_ui32NbSamples0;
	...
	for(uint32 ii=0; ii<l_rDynamicBoxContext.getInputChunkCount(0); ii++)
	{
		...
		sig.resize(l_ui32NbChannels0);
		for(uint32 i = 0; i < l_ui32NbChannels0; i++)
		{
			sig[i].resize(l_ui32NbSamples0);
		}
		...
		const uint32 l_ui32Infolength = (length[0].size()+flag[0].size()+2);
		...
		m_oAlgoInfo_SignalEncoder.getInputMatrix()->setDimensionSize(1,l_ui32Infolength);
		...
		for (uint32 o = 0; o < l_rStaticBoxContext.getOutputCount()-1; o++)
		{
			...
			m_oAlgoX_SignalEncoder[o].getInputMatrix()->setDimensionSize(0,l_ui32NbChannels0);
		...
		{
			uint32 f=0;
			m_oAlgoInfo_SignalEncoder.getInputMatrix()->getBuffer()[f+i*l_ui32Infolength]=length[i].size();
			for (uint32 l=0;l<length[i].size();l++)
			{
				m_oAlgoInfo_SignalEncoder.getInputMatrix()->getBuffer()[l+1+i*l_ui32Infolength]=length[i].at(l);
				f=l;
			}
			m_oAlgoInfo_SignalEncoder.getInputMatrix()->getBuffer()[f+2+i*l_ui32Infolength]=flag[i].size();
			for (uint32 l=0;l<flag[i].size();l++)
			{
				m_oAlgoInfo_SignalEncoder.getInputMatrix()->getBuffer()[f+3+l+i*l_ui32Infolength]=flag[i].at(l);
			}
		}

		//Store dwt coefficients to each decomposition level
		uint32 l_ui32Vector_Position=0;
		...
		for(uint32 i=0; i<l_ui32NbChannels0; i++)
		{
			...
			{  
				...
				{
					m_oAlgoX_SignalEncoder[o].getInputMatrix()->getBuffer()[l+i*length[i].at(o)] = dwt_output[i][l+l_ui32Vector_Position];
				}
				l_ui32Vector_Position=l_ui32Vector_Position+length[i].at(o);
			}
			l_ui32Vector_Position=0;
		}
