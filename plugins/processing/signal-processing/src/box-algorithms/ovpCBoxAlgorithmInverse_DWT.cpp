...
//    uint32 J = std::atoi(m_CStringDecompositionLevel);
    ...
    std::vector<uint32> l_uint32NbChannels0 (l_rStaticBoxContext.getInputCount());
    std::vector<uint32> l_uint32NbSamples0 (l_rStaticBoxContext.getInputCount());
...
//Check if all inputs have some information to decode
    if (l_rDynamicBoxContext.getInputChunkCount(l_rStaticBoxContext.getInputCount()-1)==1)
    {
        ...
        for(uint32 ii=0; ii<l_rDynamicBoxContext.getInputChunkCount(0); ii++)
        {
            ...
            l_uint32NbChannels0[0] = m_AlgoInfo_SignalDecoder.getOutputMatrix()->getDimensionSize(0);
            l_uint32NbSamples0[0] = m_AlgoInfo_SignalDecoder.getOutputMatrix()->getDimensionSize(1);
            ...
            length.resize(l_uint32NbChannels0[0]);
            flag.resize(l_uint32NbChannels0[0]);

            for(uint32 i=0; i<l_uint32NbChannels0[0]; i++)
            {
                uint32 f=0;
        ...
        if (l_flagreceveid==1)
        {
            ...
            for (uint32 o = 0; o < l_rStaticBoxContext.getInputCount()-1; o++)
            {
                ...
                for(uint32 ii=0; ii<l_rDynamicBoxContext.getInputChunkCount(o+1); ii++)
                {
                    ...
                    l_uint32NbChannels0[o+1] = m_oAlgoX_SignalDecoder[o].getOutputMatrix()->getDimensionSize(0);
                    // this->getLogManager() << LogLevel_Warning << "channels: "  << l_uint32NbChannels0[o+1] << "\n";

                    l_uint32NbSamples0[o+1] = m_oAlgoX_SignalDecoder[o].getOutputMatrix()->getDimensionSize(1);
                    ...
                    dwtop.resize(l_uint32NbChannels0[0]);

                    //Store input data (dwt coefficients) in just one vector (dwtop)
                    for(uint32 i=0; i<l_uint32NbChannels0[0]; i++)
                    {
                        for (uint32 jj=0; jj<l_uint32NbSamples0[o+1]; jj++)
                        {
                            dwtop[i].push_back(l_pBuffer[jj+i*l_uint32NbSamples0[o+1]]);
                        }
                    ...
                    if ((uint32)(length[0].size())>0 && o==l_rStaticBoxContext.getInputCount()-2)
                    {
                        ...
                        if ((uint32)(length[0].at(l_rStaticBoxContext.getInputCount()-1))==(uint32)dwtop[0].size())
                        {
                            ...
                            //Resize idwt vector
                            idwt_output.resize(l_uint32NbChannels0[0]);
                            ...
                            m_oAlgo0_SignalEncoder.getInputMatrix()->setDimensionSize(0,l_uint32NbChannels0[0]);
                            m_oAlgo0_SignalEncoder.getInputMatrix()->setDimensionSize(1,length[0].at(l_rStaticBoxContext.getInputCount()-1));



                            for (uint32 d_i=0; d_i<l_uint32NbChannels0[0]; d_i++)
                            {
