...
boolean CAlgorithmARBurgMethod::initialize(void)
{
	...
	ip_ui64Order.initialize(this->getInputParameter(OVP_Algorithm_ARBurgMethod_InputParameterId_UInteger));
...
{
	m_ui32Order = (uint32)ip_ui64Order;
	...
	if(this->isInputTriggerActive(OVP_Algorithm_ARBurgMethod_InputTriggerId_Process))
	{
		...
		for(uint32 j=0; j<l_ui32ChannelCount; j++)
		{
			...
			{
				l_ui32Length = l_ui32SamplesPerChannel - n;

				m_vecXdErrForwardPrediction.resize(l_ui32Length);
				m_vecXdErrBackwardPrediction.resize(l_ui32Length);
	
				m_vecXdErrForwardPrediction = m_vecXdErrForward.tail(l_ui32Length);
				m_vecXdErrBackwardPrediction = m_vecXdErrBackward.head(l_ui32Length);
