...
namespace{
	const char* const c_sTypeNodeName = "PLDA";
	const char* const c_sClassesNodeName = "Classes";
	const char* const c_sInvClassNodeName = "Invariant";
	const char* const c_sCoeffNodeName = "Coefficients";
	const char* const c_sCoeffWNodeName = "CoefficientsW";
	const char* const c_sCoeffW0NodeName = "CoefficientsW0";
}
...
boolean CAlgorithmClassifierPLDA::train(const IFeatureVectorSet& rFeatureVectorSet)
{
	...
	l_oMeanFeatureVector1/=static_cast<double>(l_vClassLabels[m_f64Class1]);
	l_oMeanFeatureVector2/=static_cast<double>(l_vClassLabels[m_f64Class2]);
...
boolean CAlgorithmClassifierPLDA::classify(const IFeatureVector& rFeatureVector, float64& rf64Class, IVector& rClassificationValues)
{
	...
	{
		this->getLogManager() << LogLevel_Warning << "Feature vector size " << rFeatureVector.getSize() << " and hyperplane parameter size "
							  << static_cast<uint32>(m_oW.size()) << " does not match\n";
		return false;
	}

	itpp::vec l_oFeatures(rFeatureVector.getBuffer(), rFeatureVector.getSize());

	const float64 l_f64a = (m_oW.transpose() * l_oFeatures)[0] + m_f64w0;
