...
boolean CBoxAlgorithmP300SpellerStimulator::process(void)
{
	...
	if(m_bStartReceived)
	{
		...
		else
		{
			...
			{
				l_ui32State=State_ExperimentStopRest;
			}
			else
			{
		...
		if(l_ui32State!=m_ui32LastState)
		{
			...
			switch(l_ui32State)
			{
				...
				case State_None:
					...
					_OPTIONAL_LOG_(this->getLogManager(), LogLevel_Trace << "sends OVTK_StimulationId_TrialStop\n");					
					break;
				case State_ExperimentStopRest:
					l_oStimulationSet.appendStimulation(OVTK_StimulationId_ExperimentStop, l_ui64CurrentTime + ITimeArithmetics::secondsToTime(5.0), 0);
